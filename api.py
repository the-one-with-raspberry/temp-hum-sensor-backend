            ...
            response = jsonify(
                {
                    "status": 200,
                    "content": {
                        "humidity": {"value": round(sht21.read_humidity(), 1)},
                        "farenheit": {
                            "value": round(sht21.read_temperature(), 1) * (9 / 5) + 32,
                            "unit": "farenheit",
                        },
                        "celsius": {
                            "value": round(sht21.read_temperature(), 1),
                            "unit": "celsius",
                        },
                    },
                }
...
def api_getHist():
    ...
    if time == "1d":
        ...
        response = jsonify(
            {"status": 400, "error": "The client sent an invalid time range."}
        )
        return response, 400
    if histTuple == None or type(histTuple) == str:
        response = jsonify(
            {
                "status": 500,
                "error": "A column in the database is Unbound. This isn't your fault!",
            }
        )
        return response, 500
    goofy_ahh_response = []
    for entry in histTuple:
        if histTuple.index(entry) % int(interval) == 0:
            currentTime = entry[2].replace(microsecond=0)
            currentTime = str(currentTime) + f" GMT{TIMEZONE}"
            goofy_ahh_response.append(
                {
                    "humidity": round(entry[1], 1),
                    "celsius": round(entry[0], 1),
                    "fahrenheit": round(entry[0] * 9 / 5 + 32, 1),
